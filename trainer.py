...
def trainer_synapse(args, model, snapshot_path):
    ...
    if args.enable_25D:
        max_iterations = args.max_epochs * 2 * len(trainloader)
    else:
        max_iterations = args.max_epochs * len(trainloader)
    logging.info("{} iterations per epoch. {} max iterations ".format(len(trainloader), args.max_epochs * len(trainloader)))
    best_dice = 0.0
