...
class MISSFormer(nn.Module):
    def __init__(self, enable25D, num_classes=9, token_mlp_mode="mix_skip", encoder_pretrained=True):
        super().__init__()
        self.enable25D = enable25D
        ...
        for i in range(4):
            # self.SA_layers.append(SA(dims[i]*3))
            self.SA_layers.append(nn.Conv2d(dims[i]*3, dims[i], kernel_size=1, stride=1))
    ...
    def forward(self, x, train=True, idx_slice=-1, stride=-1, length=-1):
        if self.enable25D:
            ...
            features_L = self.bridge(self.backbone(x_L))  # (B, n_patch, hidden)
            features_M = self.bridge(self.backbone(x_M))  # (B, n_patch, hidden)
            features_R = self.bridge(self.backbone(x_R))  # (B, n_patch, hidden)

            feature_L0, feature_L1, feature_L2, feature_L3 = features_L[0], features_L[1], features_L[2], features_L[3]
            feature_M0, feature_M1, feature_M2, feature_M3 = features_M[0], features_M[1], features_M[2], features_M[3]
            feature_R0, feature_R1, feature_R2, feature_R3 = features_R[0], features_R[1], features_R[2], features_R[3]
            feature_0 = self.SA_layers[0](torch.cat([feature_L0, feature_M0, feature_R0], dim=1))
            feature_1 = self.SA_layers[1](torch.cat([feature_L1, feature_M1, feature_R1], dim=1))
            feature_2 = self.SA_layers[2](torch.cat([feature_L2, feature_M2, feature_R2], dim=1))
            feature_3 = self.SA_layers[3](torch.cat([feature_L3, feature_M3, feature_R3], dim=1))
            feature_0 = feature_0 + feature_M0
            feature_1 = feature_1 + feature_M1
            feature_2 = feature_2 + feature_M2
            feature_3 = feature_3 + feature_M3
            bridge = [feature_0, feature_1, feature_2, feature_3]
        else:
            x = x.repeat(1, 3, 1, 1)
